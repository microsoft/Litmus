...
def predict_on_file(pred_filename, data_dir, save_dir, train_filename):
    ...
    for i in range(len(to_pred_data)):
        ...
        data_size_str += ",".join([f"{lang}:{size}" for lang,size in train_config.items()])
        if i != len(to_pred_data) - 1:
            data_size_str += ";"
    
    tgt_langs = list(to_pred_data[0]["eval_results"].keys())
    
    cmd = f"xlmr --scores_file {train_filename} --error_method split --pivot_features all --mode pred"
    cmd += f" --data_sizes {data_size_str}"
    cmd += f" --heatmap_targets {','.join(tgt_langs)}"
    ...
    for i in range(len(to_pred_data)):
        pred_dict.append({
            ...
            "eval_results" : {
                lang : str(pred)
                for lang,pred in zip(tgt_langs, tgt_perf_preds[i])
            }
        })
...
def main():
    ...
    pred_files = glob.glob(f"{args.data_dir}/test_release/*_XLMR*.json")
    ...
    for pred_file in pred_files:
        ...
        predict_on_file(pred_file,
            args.data_dir,
            f"{args.out_dir}/preds/",
            train_file
        )
